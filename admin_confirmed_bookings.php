...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['cancel_booking_id'])) {
    ...
    if (!isset($_POST['csrf_token']) || !validateCsrfToken($_POST['csrf_token'])) {
        ...
        if ($booking_id <= 0) {
            ...
            try {
                // Fetch the booking to verify its status and details
                $stmt = $conn->prepare("SELECT flight_id, seat_type FROM bookings WHERE booking_id = ? AND status = 'Confirmed' LIMIT 1");
                if (!$stmt) {
...
// Fetch Confirmed Bookings with Pagination
$query = "
    SELECT 
        b.booking_id,
        b.status,
...
// Get Total Number of Confirmed Bookings for Pagination
$count_query = "SELECT COUNT(*) as total FROM bookings b WHERE b.status = 'Confirmed'";
...
<head>
    ...
    <title>Admin - Confirmed Bookings - Flight Ticket Booking System</title>
    ...
    <main class="flex-grow container mx-auto px-4 py-8 max-w-7xl">
        ...
        <?php if (!empty($bookings)): ?>
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                <h2 class="text-2xl font-semibold mb-4 text-indigo-600">Confirmed Bookings</h2>
        ...
        <?php else: ?>
            <p class="text-gray-700">No confirmed bookings found.</p>
        <?php endif; ?>
    </main>
